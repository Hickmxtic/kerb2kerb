...
The Bookings panel was written to run inside a Claude.ai artifact, where `window.claude.use('mcp')` gives it a live connection to your Google Calendar. That API only exists inside claude.ai's artifact runtime — it does **not** exist on a plain static host like the one this is served from.
...
If you want live bookings back on the standalone version, that needs a real backend integration with the Google Calendar API (OAuth + a server-side call — a small serverless function on whatever host is chosen) — a different, larger piece of work from the rest of this migration.
